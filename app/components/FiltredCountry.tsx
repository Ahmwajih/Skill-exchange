...
const FiltredCountry: React.FC<{ searchResults: any[] }> = ({ searchResults }) => {
  ...
  useEffect(() => {
    if (selectedCountry === "All") {
      ...
      setFilteredSkills(searchResults.filter(skill => skill.userId.country.toLowerCase() === selectedCountry.toLowerCase()));
    }
      ...
      <div className="container sm:mx-2 bg-white mx-auto lg:px-2 mt-6">
        <div className="grid cardContainer sm:mx-2 md:grid-cols-2 sm:grid-cols-1 lg:grid-cols-3 gap-6 justify-items-center">
          {filteredSkills.length > 0 ? (
            ...
            <p className="text-gray">No skills found for the selected country.</p>
          )}
        </div>
      </div>
    </div>
